...
\textbf{Implementation details.} For a fine-tuned target we generate 4{,}096 responses with that target, cache its hidden states, sample 32 anchors per example, and train one epoch, which is 512 optimizer updates at global batch 8 with AdamW at learning rate $10^{-4}$. For a replaced target we reuse 16{,}384 saved NuminaMath~\citep{li2024numinamath} trajectories, supervise a fixed 25\% of prompt and response positions, and train three epochs, which is 6{,}144 updates at learning rate $10^{-3}$. Both use seed 42 on two L40S GPUs with FP32 trainable maps over a frozen BF16 model, and both export the folded projection in BF16. Single-request measurements use Transformers on one L40S with greedy non-thinking decoding, CUDA-synchronized timing and warmup excluded. Serving measurements use vLLM 0.28.0 with batch-invariant operations. In the selected single-request comparisons the unadapted, native and adapted drafters returned identical output sequences on all 128 prompts per workload, so those ratios are not confounded by output length. Under vLLM the sequences can diverge and we treat those numbers as descriptive.
...
\subsection{Additional hardware and workload coverage}
\label{sec:future-hw}
The results above use one L40S GPU for single-request timing and vLLM on L40S for concurrent serving. Three extensions are planned and not yet measured: throughput and memory behavior on DGX Spark and A100 hardware, multi-GPU tensor-parallel serving of larger targets, and adaptation to long-context workloads beyond the 8{,}192-token cap used for KiCad. These extensions test whether the recovery and cost trends of Sections~\ref{sec:main-results}--\ref{sec:related-work-compare} hold on different accelerators, under tensor parallelism, and at longer sequence lengths, and are left for a revision.
