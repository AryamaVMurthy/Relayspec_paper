...
\textbf{Metrics.} \emph{Speedup} is mean per-request throughput divided by that of autoregressive decoding of the same target on the same prompts, so autoregressive decoding is $1.00\times$ by definition, and \emph{acceptance length} $\tau$ is pooled emitted tokens per verification step including the correction token. Aggregate serving throughput divides total returned tokens by workload wall time including drain. These statistics weight requests differently and we never mix them inside one comparison. Where an ablation must choose one configuration we select by the harmonic mean of its per-workload speedups, because it is pulled down by the smallest entry and will not let a configuration hide one weak workload behind a favourable average.
...
\label{sec:ablations}
We ablate the three decisions that define the method; Appendix~\ref{app:ablations} reports all of them in full. Capacity is settled quickly: adding a zero-initialized nonlinear branch $B_i\operatorname{SiLU}(A_ih_i)$~\citep{elfwing2018silu} to each map changes throughput by between $-2.07\%$ and $+2.97\%$ and is behind the linear map on five of seven workloads while carrying 16--20\% more parameters and staying active at inference.

\textbf{Where the trained parameters sit.} Training one matrix of the same total size in place of the five maps, initialized to exactly the function they start from, reaches a harmonic mean of 1.265 against 1.376 and loses 45 points of gain on KiCad; transferring to Qwen3-8B it reaches $0.365\times$ the native drafter on Math against $0.884\times$. The factorization does the work, not the parameter count, as structural re-parameterization predicts~\citep{huo2026repspec}. Low-rank replacement of each whole map degrades monotonically, and draft-transformer LoRA is competitive and wins on NanoCoder, so we do not claim input adaptation beats body adaptation everywhere; its cost is that the draft transformer is no longer shared.
...
\textbf{How much supervision the maps need.} Figure~\ref{fig:scaling} varies both supervision axes across three orders of magnitude. Adaptation is effective from as few as 16 examples, and throughput rises smoothly without saturating out to 16{,}384: GSM8K goes from $+10.8\%$ to $+45.0\%$ and KiCad from $+5.0\%$ to $+113.0\%$, while transfer recovery rises from 15.6\% to 99.9\% on Math and 18.9\% to 84.8\% on Code. The main results are therefore conservative operating points rather than ceilings.

\input{figures/scaling_figure}

\subsection{Generalization, comparison and cost}
\label{sec:eagle}
\textbf{A second drafter family.} We repeat both settings on published EAGLE-3 checkpoints, which read three target streams and draft one token at a time, pairing each setting with the objective selected above. Adapting a frozen EAGLE-3 drafter improves it in both settings, by 45.9\% to 105.7\% on fine-tuned targets and above a purpose-built Qwen3-8B EAGLE-3 drafter on all four transfer workloads, reaching $3.82\times$, $3.75\times$ and $5.00\times$ autoregressive on GSM8K, NanoCoder and KiCad against $2.26\times$, $2.57\times$ and $2.43\times$ unadapted (Appendix~\ref{app:eagle-table}). The objective selection replicates: reconstruction on the same fine-tuned targets gives 71.89, 75.34 and 78.70 tokens per second, a 7.9\% regression on NanoCoder, while token supervision gives 120.35, 119.30 and 151.86. Pairing the objective to the setting is therefore not a DFlash artifact.

\textbf{Published systems, serving and cost.} An earlier single-projection version of this interface, compared on 128 MATH development questions against PARD~\citep{an2026pard} and frozen-drafter SD$^2$~\citep{berdoz2026steering}, each measured against its own runtime's autoregressive baseline, reaches $5.14\times$ [4.90, 5.37] against $3.34\times$ and $1.60\times$ (Appendix~\ref{app:public-runtime}). Under vLLM with three LoRA targets sharing one draft backbone, aggregate throughput is higher for the adapted drafter at every concurrency, 4{,}324 tokens per second against 2{,}440 unadapted and 828 autoregressive at 32 clients; one pooled set of maps reaches 98.9--99.6\% of three separate sets, and adaptation adds a constant 0.33 GiB. Against the published DFlash recipe~\citep{chen2026dflash} our recipes use 195 and 49 times fewer examples, each fitting in under half an hour (Appendix~\ref{app:ablations}).
