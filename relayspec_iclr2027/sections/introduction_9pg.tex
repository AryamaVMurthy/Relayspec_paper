...
This dependence complicates reuse when a service changes its target's size or family, or fine-tunes it with methods such as LoRA~\citep{hu2022lora}: the new target produces different hidden states and preferred continuations, and changed feature dimensions may be incompatible with the existing drafter. Training a replacement requires target-specific supervision, optimization of the drafter's weights and validation~\citep{li2025eagle3,chen2026dflash}, and repeating this across target variants increases training and maintenance costs. Can we instead reuse an existing drafter by adapting only its target-feature inputs?

We introduce RelaySpec, a method for reusing a pretrained DFlash drafter~\citep{chen2026dflash} after its target changes (Figure~\ref{fig:relayspec-overview}). The new target supplies hidden states from five layers, a separate linear map transforms each stream before the drafter's original fusion projection combines them, and the resulting context conditions the drafter transformer, which predicts a block of candidate tokens. Section~\ref{sec:preliminaries} explains the parts of DFlash that RelaySpec uses.

During adaptation only the five maps are trained; the target, fusion projection, drafter transformer, normalization layers, embeddings and output head all remain frozen. A map can be rectangular when the target and drafter expect different feature dimensions. Before inference we multiply each map into its fusion block, producing one effective projection, so inference needs no separate mapping operation.
...
\begin{itemize}
    \item We introduce RelaySpec, an adapter between a target's hidden-state representations and a pretrained DFlash drafter: five trainable linear maps that keep the drafter transformer frozen and fold into the original fusion projection before inference.
    \item We evaluate adaptation across fine-tuned targets, model sizes and families, with training-budget and shared-serving comparisons. On Qwen3-4B LoRA targets the maps improve mean per-request throughput over the unchanged drafter by 39.51\% and 85.47\% on GSM8K~\citep{cobbe2021gsm8k} and KiCad, and the Qwen3-4B-to-8B transfer recovers 100.2\% and 99.4\% of native-drafter throughput on MATH and GSM8K, with weaker transfer on code and chat.
    \item We analyze the learned maps through SVD truncation, correction scaling, alignment with LoRA-induced feature changes, and interpolation toward MSE-trained maps.
\end{itemize}
