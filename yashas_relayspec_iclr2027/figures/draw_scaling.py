"""Plot measured throughput relative to the setting's comparison drafter."""
...
# --- LoRA-fine-tuned targets: percent gain over the unadapted drafter (phase 1 scaling export)
...
json.dump({'lora_metric':'mean per-request throughput as a percentage of the unadapted drafter',
 'cross_transfer_metric':'mean per-request throughput as a percentage of the native drafter',
 ...
 'lora_examples':[{'domain':d,'examples':n,'percent_unadapted':100+(SF[(d,'maps',n)] if n in SM else F[(d,'maps',n,32)])} for d,_,_ in DOM for n in ALL_FT],
 'lora_anchors':[{'domain':d,'arch':a,'anchors':k,'percent_unadapted':100+F[(d,a,4096,k)]} for d,_,_ in DOM for a in ('maps','body') for k in AN],
 'cross_transfer_examples':[{'examples':n,'workload':w,'percent_native':100*(SC[(n,w)] if n in SM else C[(n,w)])} for n in ALL_T1 for w,_,_ in WL]},
...
for d,lab,col in DOM:
 y=[100+(SF[(d,'maps',n)] if n in SM else F[(d,'maps',n,32)]) for n in ALL_FT]
 a.plot(range(len(ALL_FT)),y,'-o',color=col,ms=3.2,lw=1.5,label=lab)
a.axhline(100,color='#707070',lw=.7,ls='--',zorder=0)
a.set(xticks=range(len(ALL_FT)),xticklabels=[f'{x//1024}k' if x>=1024 else str(x) for x in ALL_FT],
      xlabel='Example presentations',ylim=(0,240))
a.set_ylabel('Throughput vs unadapted (%)')
a.set_title('(a) LoRA-fine-tuned',loc='left',fontsize=8,weight='bold')
a.legend(fontsize=6.4,frameon=False,loc='upper left',handlelength=1.5)
a.tick_params(axis='x',labelrotation=35)

for d,lab,col in DOM:
 y=[100+F[(d,'maps',4096,k)] for k in AN]
 yb=[100+F[(d,'body',4096,k)] for k in AN]
...
b.axhline(100,color='#707070',lw=.7,ls='--',zorder=0)
b.set(xticks=range(len(AN)),xticklabels=[str(x) for x in AN],xlabel='Anchors per example (4,096 ex.)',ylim=(0,240))
b.set_ylabel('Throughput vs unadapted (%)')
b.set_title('(b) LoRA-fine-tuned',loc='left',fontsize=8,weight='bold')
...
c.set(xticks=range(len(ALL_T1)),xticklabels=[f'{x//1024}k' if x>=1024 else str(x) for x in ALL_T1],
      xlabel='Training examples',ylim=(0,108))
c.set_ylabel('Throughput vs native (%)')
c.set_title('(c) Cross-transfer: examples',loc='left',fontsize=9,weight='bold')
