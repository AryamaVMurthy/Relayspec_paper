...
\begin{figure}[t]
    ...
    \caption{RelaySpec training and inference. Orange maps are trainable and blue model components are frozen. Each hidden-state matrix $H_i$ enters a map $W_i$ before fusion. A single token gives $W_i h_{i,t}$. Here $n$ is the number of tokens, $d_t$ is the new target's hidden-state width, $d_r$ is the width expected by the reused fusion projection, and $d_d$ is the drafter transformer's hidden width. For Qwen3-8B with the Qwen3-4B drafter, $d_t=4096$ and $d_r=d_d=2560$. Each $W_i$ is $2560\times4096$, each $F_i$ is $2560\times2560$, and the folded fusion projection is $2560\times20480$. The new target verifies proposals after adaptation.}
